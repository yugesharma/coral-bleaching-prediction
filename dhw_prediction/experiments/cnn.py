...
from src.data.dataset import ProfileDataset
from src.data.preprocessing import fit_scalers, apply_scalers, save_scalers
from src.models import CNN1DWithAttention
...
from src.evaluation.plots import (
    ...
    plot_training_history, plot_attention_weights
)

RESULTS_DIR = 'experiments/runs/cnn_attention'
os.makedirs(RESULTS_DIR, exist_ok=True)

DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
DEPTH_LEVELS = np.arange(2, 302, 2)  # 2m to 300m


def get_probs_and_attention(model, loader, device):
    """Get predicted probabilities and attention weights from CNN."""
    model.eval()
    all_probs   = []
    all_labels  = []
    all_attn    = []
...
def run():
    ...
    test_ds      = ProfileDataset(test_df)
    ...
    y_val_probs,  y_val,  val_attn  = get_probs_and_attention(
        model, val_loader,  DEVICE
    )
    y_test_probs, y_test, test_attn = get_probs_and_attention(
    ...
    plot_roc_curve(y_test, y_test_probs, 'CNN+Attention', figures_dir)
    plot_pr_curve(y_test, y_test_probs, 'CNN+Attention', figures_dir)
    plot_confusion_matrix(
        y_test, y_test_probs,
        val_metrics['threshold'], 'CNN+Attention', figures_dir
    )
    plot_training_history(history, 'CNN+Attention', figures_dir)

    pos_mask = y_test == 1
    neg_mask = y_test == 0

    plot_attention_weights(
        test_attn[pos_mask], DEPTH_LEVELS,
        'CNN+Attention_bleaching', figures_dir
    )
    plot_attention_weights(
        test_attn[neg_mask], DEPTH_LEVELS,
        'CNN+Attention_no_bleaching', figures_dir
    )
